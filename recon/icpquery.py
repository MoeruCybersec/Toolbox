...
class Crack:
    ...
    def detect(self, big_img):
        ...
        iou_thres = 0.7
        session = onnxruntime.InferenceSession(os.path.join(script_path, "yolov8.onnx"))
    ...
    def siamese(self, small_img, boxes):
        session = onnxruntime.InferenceSession(
            os.path.join(script_path, "siamese.onnx")
        )
...
def generate_pointjson(big_img, small_img, secretKey):
    ...
    enc_pointJson = aes_ecb_encrypt(
        json.dumps(pointJson).replace(" ", "").encode(), secretKey.encode()
    )
...
def query(sign, uuid_token, domain, pageSize, pageNum):
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36",
        "Referer": "https://beian.miit.gov.cn/",
        "Token": token,
...
#     params["bigImage"], params["smallImage"], params["secretKey"]
# )
# time.sleep(0.5)
...
def run(keyword):
    ...
    while pages is None or pageNum <= pages:
        if sign:
            result = query(
                sign,
                params["uuid"],
